...
public class Checkpoints : MonoBehaviour {
    ...
    public void NextCheckpoint(int index)
    {
        ...
        arrowTemp = Instantiate(arrow, transform.GetChild(index).position, Quaternion.LookRotation(transform.GetChild(index + 1).position - transform.GetChild(index).position), transform.GetChild(index));
